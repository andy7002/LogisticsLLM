...
class DataBaseHackerEarthOneHot(DataBaseABC):
    def __init__(self, feature_lst, feature_en_lst, key_lst, order_strategy, sequence, split, cot, scale):
        super().__init__('HEOneHot', feature_lst, feature_en_lst, key_lst, order_strategy, sequence, split, cot, scale)
    ...
    def one_hot_encode(self):
        cols_to_be_one_hot_encoded = ["Material", "Transport", "Customer Location", "Month", "Year", "Week"]
        cols_to_be_ordinally_encoded = ["International", "Express Shipment", "Installation Included", "Fragile",
                                        "Customer Information", "Remote Location"]

        for i in cols_to_be_one_hot_encoded:
            self.data = self.data.join(pd.get_dummies(self.data[i], prefix=i).astype(int))

        for i in cols_to_be_ordinally_encoded:
            self.data = self.ordinal_encoding(self.data, i)

    def ordinal_encoding(self, df, column):
        ordinal_encoder = ce.OrdinalEncoder(cols=column, return_df=True, mapping=[
            {'col': column, 'mapping': {'Yes': 1, 'No': 0, 'Working Class': 0, 'Wealthy': 1}}])
